...
int calculate_perimeter(const Map& map, Position p)
{
	...
	for (Position p_neighbor : {up(p), down(p), left(p), right(p)})
	{
		if (!in_bounds(map, p_neighbor) ||
			plant != map[p_neighbor.y][p_neighbor.x].plant)
		{
...
int count_corners(const Map& map, Position p)
{
	...
	bool p_up_is_perimeter =
		!in_bounds(map, p_up) ||
		plant != map[p_up.y][p_up.x].plant;
	bool p_down_is_perimeter =
		!in_bounds(map, p_down) ||
		plant != map[p_down.y][p_down.x].plant;
	bool p_left_is_perimeter =
		!in_bounds(map, p_left) ||
		plant != map[p_left.y][p_left.x].plant;
	bool p_right_is_perimeter =
		!in_bounds(map, p_right) ||
		plant != map[p_right.y][p_right.x].plant;

	corners += (p_up_is_perimeter && p_left_is_perimeter) ||
		((!p_up_is_perimeter && !p_left_is_perimeter) && plant != map[p_up_left.y][p_up_left.x].plant);
	corners += (p_up_is_perimeter && p_right_is_perimeter) ||
		((!p_up_is_perimeter && !p_right_is_perimeter) && plant != map[p_up_right.y][p_up_right.x].plant);
	corners += (p_down_is_perimeter && p_left_is_perimeter) ||
		((!p_down_is_perimeter && !p_left_is_perimeter) && plant != map[p_down_left.y][p_down_left.x].plant);
	corners += (p_down_is_perimeter && p_right_is_perimeter) ||
		((!p_down_is_perimeter && !p_right_is_perimeter) && plant != map[p_down_right.y][p_down_right.x].plant);
	return corners;
}
...
Region walk_region(Map& map, Position p)
{
	Region r = {
		.area = 1,
		.perimeter = calculate_perimeter(map, p),
		.corners = count_corners(map, p)
	};
