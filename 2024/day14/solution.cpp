...
Robots parse_input(std::istream& input)
{
    ...
    for (std::string line; std::getline(input, line);)
    {
        Robot robot;
        sscanf(
            line.data(), "p=%d,%d v=%d,%d", &robot.position.x, &robot.position.y, &robot.velocity.x,
            &robot.velocity.y);
...
{
    std::print(
        "p={},{} v={},{}\n", robot.position.x, robot.position.y, robot.velocity.x,
        robot.velocity.y);
}
...
int variance(Robots& robots)
{
    ...
    for (auto& robot : robots)
    {
        ...
        {
            variance += (abs(other.position.x - robot.position.x) +
                         abs(other.position.y - robot.position.y)) /
                        2;
        }
    }
    return variance;
