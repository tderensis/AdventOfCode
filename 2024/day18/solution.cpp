...
std::tuple<Map, Position, Position> parse_input(std::istream& input, int size, int bytes)
{
    Map      map;
    Position start{0, 0};
...
bool visit_neighbors(Map& map, std::vector<Node>& open_nodes, const Node& node, Position end)
{
    ...
    for (Direction d : {Direction::up, Direction::down, Direction::left, Direction::right})
    {
        ...
        if (in_bounds(map, p) && object_at(map, p) == EMPTY)
        {
            ...
            object_at(map, p) = (d == Direction::up)     ? '^'
                                : (d == Direction::down) ? 'v'
                                : (d == Direction::left) ? '<'
                                                         : '>';
        }
